...
//
// 结构: 16x16 部分积矩阵 (32 列), 最低 12 列 (列 0..11) 的部分积直接
//       丢弃(不生成、不补偿), 其余部分积用标准 Dadda 树精确压缩。
//       由于保留部分的归约是精确的, 功能上逐位等价于:
//         P = sum_{i=0..15} a_i ? ((b << i) & (0xFFFFFFFF << 12)) : 0
//
// 无符号核心 MRED = 1.6e-04 (2^24 组均匀随机 uint16 输入), 误差恒为非负。
//
...
// 无符号核心: 纯截断 Dadda, 丢弃部分积第 0..11 列
__device__ uint32_t dadda16u_trunc(uint16_t a, uint16_t b)
{
    const uint32_t keep = 0xFFFFFFFFu << 12;
    uint32_t p = 0;
    #pragma unroll
    for (int i = 0; i < 16; ++i)
        if ((a >> i) & 1u)
