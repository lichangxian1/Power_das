...
展示口径：
  ...
  * 背景基线：纯截断 Dadda 种子（主 run 快照内 n_cells==0 每 k 首次入档）+
    纯截断 arith（一个或多个 --trunc_dir 的 k*/best_info.json，按 MRED 合成一条线，
    与 plot_v5_progress 同口径）；
...
用法: python scripts/gen_v5_front_viewer.py <fronts_root> [out.html]
        ...
        [--trunc_dir outputs/first --trunc_dir outputs/second]
"""
import argparse
import json
import os
...
# 新纯截断 arith 基线的两段任务：MRED-C* 的偶数 k02..k20 与 deep-k 的 k22..k30。
DEFAULT_TRUNC_DIRS = (
    "outputs/2026-07-09_mred_trunc_baseline",
    "outputs/2026-07-11_03_mred_deepk_trunc_np4",
)


def _three_stage_entries(items):
    """Map Candidate dictionaries to the compact fields consumed by the viewer."""
    entries = []
    for item in items:
        if not item.get("valid", True):
            continue
        try:
            mred = float(item["mred"])
        ...
        entries.append({
            ...
            "power": power,
            "k": int(item.get("k", -1)),
            "n_cells": int(item.get("n_cells", len(item.get("cells") or []))),
            "bin": item.get("bin", metadata.get("selection_bin", item.get("stage", -1))),
        })
    return entries


def _three_stage_item_generation(item):
    value = (item.get("metadata") or {}).get("generation", 0)
    try:
        return int(value)
...
def _three_stage_generation(items):
    return max((_three_stage_item_generation(item) for item in items), default=0)


def _three_stage_stage3_offset(root):
    """Return the global-episode offset where Stage 3 starts, if applicable."""
    config_path = os.path.join(root, "three_stage_config.json")
    if not os.path.isfile(config_path):
        return None
    with open(config_path) as f:
        config = json.load(f)
    return (
        int(config.get("stage1_generations", 120))
        + int(config.get("stage2_generations", 120))
    )


def collect_three_stage_runs(root):
    """Collect the live archives produced by trainer/arith_three_stage."""
    config_path = os.path.join(root, "three_stage_config.json")
    if not os.path.isfile(config_path):
        return []
    with open(config_path) as f:
    ...
    offsets = {
        1: 0,
        2: int(config.get("stage1_generations", 120)),
        3: (
            int(config.get("stage1_generations", 120))
            + int(config.get("stage2_generations", 120))
        ),
    ...
    runs = []
    for stage in (1, 2, 3):
        stage_dir = os.path.join(root, f"stage{stage}")
        if not os.path.isdir(stage_dir):
            continue
        snaps = {}
        history_dir = os.path.join(stage_dir, "front_hist")
        if os.path.isdir(history_dir):
            for filename in os.listdir(history_dir):
                match = re.match(r"front_gen(\d+)\.json$", filename)
                if not match:
                    continue
                with open(os.path.join(history_dir, filename)) as f:
                    snaps[offsets[stage] + int(match.group(1))] = _three_stage_entries(
                        json.load(f)
                    )
        archive_path = os.path.join(stage_dir, "archive.json")
        if os.path.isfile(archive_path) and not snaps:
            # No synthetic history: without persisted snapshots, expose only the
            # current exact archive as a single frame.
            with open(archive_path) as f:
                items = json.load(f)
            generation = _three_stage_generation(items)
            snaps[offsets[stage] + generation] = _three_stage_entries(items)
        if snaps:
            runs.append((f"stage{stage}", dict(sorted(snaps.items()))))

    final_path = os.path.join(root, "final", "pareto.json")
    # A persisted Stage-3 history is the source of truth for replay. In
    # particular, do not inject an older final/pareto.json at the new target
    # episode while an extended run is still in progress. The final file is
    # retained only as a compatibility fallback for legacy runs without any
    # Stage-3 snapshots.
    has_stage3_history = any(name == "stage3" for name, _snaps in runs)
    if os.path.isfile(final_path) and not has_stage3_history:
        with open(final_path) as f:
            items = json.load(f)
        final_ep = (
            offsets[3]
            + int(config.get("stage3_elites", 24))
            * int(config.get("stage3_episodes_per_elite", 5))
        )
        final_entries = _three_stage_entries(items)
        runs.append(("stage3-final", {final_ep: final_entries}))
    return runs
...
def collect_runs(root):
    ...
    return runs + collect_three_stage_runs(root)


def load_trunc_arith(base):
    """单段纯截断 arith：k*/best_info.json → [[mred, area, power_W, k]]。"""
    pts = []
...
def pack_run(name, group, cl, cd, snaps, static=False, active_until=None):
    ...
    if active_until is not None:
        packed["active_until"] = int(active_until)
    return packed
...
def main():
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="PATH",
        help=("纯截断 arith 基线目录；可重复传入，所有点按 MRED 合成一条线。"
              "未指定时使用 MRED-C* baseline + deep-k。"),
    )
    ...
    if not main_runs:
        raise SystemExit(
            f"未在 {a.root} 下找到 v5 front.json/front_ep*.json 或三阶段 archive.json"
        )
    ...
    data = {"title": title, "runs": [],
            ...
            "baselines": []}
    main_stage3_offset = _three_stage_stage3_offset(a.root)
    for i, (name, snaps) in enumerate(main_runs):
        cl, cd = DOT_COLORS[i % len(DOT_COLORS)]
        active_until = (
            main_stage3_offset if name in {"stage1", "stage2"} else None
        )
        data["runs"].append(
            pack_run(name, "GA", cl, cd, snaps, active_until=active_until)
        )
    ...
    for j, (spec, final_only) in enumerate(specs):
        ...
        compare_stage3_offset = _three_stage_stage3_offset(path)
        for name, snaps in cruns:
            rname = label if len(cruns) == 1 else f"{label}:{name}"
            active_until = (
                compare_stage3_offset
                if not final_only and name in {"stage1", "stage2"}
                else None
            )
    ...
    trunc_dirs = list(DEFAULT_TRUNC_DIRS) if a.trunc_dir is None else a.trunc_dir
    trunc_by_k = {}
    loaded_trunc_dirs = []
    for trunc_dir in trunc_dirs:
        if not trunc_dir:
            continue
        pts = load_trunc_arith(trunc_dir)
        if not pts:
            print(f"  [warn] 纯截断 arith 基线目录不可用或没有完整点，跳过: {trunc_dir}")
            continue
        loaded_trunc_dirs.append(trunc_dir)
        for pt in pts:
            k = pt[3]
            if k in trunc_by_k:
                print(f"  [warn] 纯截断 arith 的 k{k:02d} 重复，采用后传目录: {trunc_dir}")
            trunc_by_k[k] = pt
    ta = sorted(trunc_by_k.values(), key=lambda p: p[0])
    if ta:
        data["baselines"].append({"name": "纯截断 arith", "kind": "arith",
                                  "cl": VIOLET[0], "cd": VIOLET[1], "pts": ta})
        print(f"  baseline trunc-arith: {len(ta)} pts "
              f"({' + '.join(loaded_trunc_dirs)})")
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  .legend .line-toggle { display: inline-flex; align-items: center; border: 0;
                         border-radius: 6px; padding: 3px 6px; margin: -3px -6px;
                         color: inherit; background: transparent; font: inherit;
                         cursor: pointer; transition: color .12s, background .12s,
                         opacity .12s; }
  .legend .line-toggle:hover { color: var(--text-1); background: var(--grid); }
  .legend .line-toggle:focus-visible { outline: 2px solid #2a78d6;
                                       outline-offset: 2px; }
  .legend .line-toggle.active { color: var(--text-1); background: var(--grid);
                                font-weight: inherit; }
  .legend.has-focus .line-toggle:not(.active) { opacity: 0.5; }
...
XA 终审另算。点击图例中的线名称可聚焦，再次点击恢复。</div>
...
const focusOrder = (items, idOf) => [...items].sort((a, b) =>
  Number(isFocused(idOf(a))) - Number(isFocused(idOf(b))));
...
const snapAt = (r, ep) => {
  ...
  if (r.active_until != null && ep >= r.active_until) return null;
...
// Do not spread the full replay history into Math.min/Math.max. Long-running
// campaigns can exceed the browser's function-argument limit and leave the
// embedded viewer blank before the first SVG is drawn.
...
function drawBaselines() {
  ...
  for (const p of PANELS) {
    p.baseG.textContent = "";
    for (const b of focusOrder(DATA.baselines, baselineLineId)) {
      const c = col(b);
      const id = baselineLineId(b), focus = isFocused(id), faded = fadedByFocus(id);
      el("polyline", { points: b.pts.map(q => X(q[0]) + "," + p.Y(q[p.yi])).join(" "),
        fill: "none", stroke: c, "stroke-width": 1.8,
        opacity: focus ? 1 : faded ? 0.14 : 0.55,
      ...
      for (const q of b.pts) {
        ...
        if (b.kind === "dadda")
          el("circle", { cx: x, cy: y, r: 3, fill: css("--panel"),
            ...
            opacity: focus ? 1 : faded ? 0.14 : 0.7 }, p.baseG);
        else
          el("rect", { x: x - 2.8, y: y - 2.8,
            width: 5.6, height: 5.6, rx: 1.5,
            fill: c, opacity: focus ? 1 : faded ? 0.14 : 0.7 }, p.baseG);
  ...
  for (const p of PANELS) {
    ...
    for (const g of focusOrder(DATA.groups, groupLineId)) { // 聚焦线最后画，保持在最上层
      ...
      el("polyline", { points: env.map(q => X(q[0]) + "," + p.Y(q[p.yi])).join(" "),
        ...
        opacity: focus ? 1 : faded ? 0.16 : isStatic ? 0.4 : 1,
...
function legend() {
  ...
  const lineItem = (node, label, id) => {
    const button = document.createElement("button");
    button.type = "button"; button.className = "line-toggle" + (isFocused(id) ? " active" : "");
    button.title = isFocused(id) ? "再次点击取消聚焦" : "点击聚焦这条线";
    button.setAttribute("aria-pressed", String(isFocused(id)));
    button.append(node, label);
    button.addEventListener("click", () => {
      focusedLine = isFocused(id) ? null : id;
      legend(); drawBaselines(); drawGhost(); draw(+slider.value);
    });
    lg.appendChild(button);
  };
  ...
  for (const g of DATA.groups) {
    ...
    lineItem(k, g.label, groupLineId(g));
  }
  for (const r of DATA.runs) {
  ...
  for (const b of DATA.baselines) {
    if (b.kind === "dadda") lineItem(key(col(b)), b.name, baselineLineId(b));
    else { const sq = document.createElement("span"); sq.className = "sq"; sq.style.background = col(b); lineItem(sq, b.name, baselineLineId(b)); }
  }
  const gk = key(css("--text-2")); gk.style.opacity = 0.45;
  item(gk, "淡色 = 各组最终前沿参照");
  lg.classList.toggle("has-focus", focusedLine !== null);
}
