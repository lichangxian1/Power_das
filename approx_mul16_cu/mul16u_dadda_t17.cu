...
//
// 结构: 16x16 部分积矩阵 (32 列), 最低 17 列 (列 0..16) 的部分积直接
//       丢弃(不生成、不补偿), 其余部分积用标准 Dadda 树精确压缩。
//       由于保留部分的归约是精确的, 功能上逐位等价于:
//         P = sum_{i=0..15} a_i ? ((b << i) & (0xFFFFFFFF << 17)) : 0
//
// 无符号核心 MRED = 3.7e-03 (2^24 组均匀随机 uint16 输入), 误差恒为非负。
//
...
// 无符号核心: 纯截断 Dadda, 丢弃部分积第 0..16 列
__device__ uint32_t dadda16u_trunc(uint16_t a, uint16_t b)
{
    const uint32_t keep = 0xFFFFFFFFu << 17;
    uint32_t p = 0;
    #pragma unroll
    for (int i = 0; i < 16; ++i)
        if ((a >> i) & 1u)
