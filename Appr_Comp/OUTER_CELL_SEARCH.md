...
**性能优化与 4M 否决(07-11 凌晨)**:
- 每集 15min(GA 对照 4.9min),其中求解 10.5min:鲁棒修复逐摘(~30 轮×8 树)+
  升级扫描(被修复摘掉,白做)+ GPU 同卡抢占是三大头。
- **4M 池方案被探针否决**(`probe_4m_vs_16m.py`):4M/16M 读数偏差 0.81–1.70×
  (MRED 重尾,小池前缀读不准),保 16M 闸门必过需 safety=0.58 → 包被掐死;
  且全量 gate 大头是 S3 子样本(262k,不随池缩),4M 只加速次要部分。**池保持 16M**。
- **fast-solver(已实现)**:鲁棒模式跳过升级扫描 + 修复改二分批量摘除(按解析贡献
  排序二分最小前缀,摘除序列与逐摘数学同序)+ 池/估计器跨 episode 缓存
  (`GradientCellSolver(est=...)`)。
- **k14 同卡 A/B**(`2026-07-11_00_mred_greedy_fast_k14`,cuda:0 与慢版同卡同 seed):
  求解阶段 10.9–11.1 → **5.9–7.7min(~1.7×)**,每集 ~14.9 → ~11min;包 9–24 cell/
  util 86–96%/over_budget 全 0(与慢版同量级,无质量回退迹象)。剩余大头 = 初始堆
  ~900 次单 cell 打分(未向量化)+ 同卡 5 作业抢占;进一步提速需批量向量化候选评估。
...
### 3.2.6 代理→真实落差定罪(2026-07-11,"为什么兑现率只有 18–62%")

**H1(锚点/单位虚高)= 主犯,已量化闭合**:
- 代理的 exact 锚点来自"真值表 SOP 单独综合"(FA=10.92/HA=8.74µm²),而整设计里 DC
  直接映射 T28 原生标准单元:**FA1D0=2.856、HA1D0=2.184(LEF 实测)——虚高 3.8–4.0×**。
- 物理反证:k12 exact 网表 204FA+14HA,按锚点算光压缩树 2385µm² > 整设计实测 726µm²
  的 3 倍;按原生单元算 648µm² 正好吻合。
- 数值闭合:全体按 FA 比例 0.262 缩放 → 预测 k12 真实省 49.7,实测 49.4。
  **兑现率 18–26% ≈ 1/3.8 不是巧合,是单位换算。**

**DC 探针(7 变体,k12 单 cell/同型组/前缀)**:
| 变体 | 代理省 | 真实省 | 每cell真实 | 原生口径预测(2.856−标称×0.262) |
|---|---|---|---|---|
| 5500 ×1/×6 | 10.6/63.5 | 3.0/20.0 | 3.0–3.3 | 2.77 ✓ |
| fe54 ×1/×12 | 6.7/80.6 | 1.7/27.1 | 1.7–2.3 | 1.76 ✓ |
| a994 ×1/×26 | 1.0/26.2 | **−3.4**/10.8 | **≈0(单点为负)** | 0.26 ✓ |
| prefix15 | 124.0 | 44.4 | 2.96 | — |
| full49 | 189.7 | 49.4 | 1.01 | — |

- **H2 逐 cell 边际:原生口径预测公式命中全部三族**——真实边际 = 原生exact −
  标称×0.262,a994 类"代理小赚"cell 真实≈0 甚至为负;
- **H3 饱和:次要**——组≈n×单(可加性成立),prefix15 已捕获整包 90% 真实节省,
  后 34 个 cell(26 个 a994)只添 5µm² 却烧掉大量误差预算。
- 功耗侧:fe54×12 真省(−2.4%),a994×26/5500×6 不省(+0.4%)。

**修正方案(待做)**:solver 面积表换**原生口径**——exact 锚点用 2.856/2.184/5.712,
近似侧用 标称×0.262 一阶折算(或用探针逐族标定)。菜单图
(`outputs/2026-07-11_cell_pareto/`)显示换口径后:真正有利可图的只剩近似常数类
cell(5500/ff55/e4 等),a994/b994 类应被剔除,T42 库面积轴收益≈0。

### 3.2.7 sub-std 重选型(2026-07-11,`select_substd_cells.py`)

选型准则倒置:**standalone 面积 < 原生标准单元**(FA1D0 2.856/HA1D0 2.184/2×FA1D0
5.712)——即使按虚高口径记账,进设计后也必然小于原生 exact,真实节省有硬保证;
代价 = 误差档位放宽(单次使用 wae 下限 0.0156→0.375)。全库扫描结果:

- **T32:3723 选 6**(canon 去重):fa50(wae.375/Z/面积0,严格优于旧 zero)、
  5500(.5/N)、aa00(.53/N)、5555(.59/P)、ff00(.75/Z/0)、ff55(.84/P)——2P/2N/2Z 平衡;
- **T22:161 选 10,建议菜单 6**:e4(.25/P/0)+50(.25/N)为 wae=0.25 的 P/N 对、
  a0(.375/Z)、00(.5/N)、f0(.625/P/0)、55(.625/P);高 wae 尾巴(f5/aa/fa/ff)弃;
- **T42:1999 全军覆没(0 合格)**——comp42n 标称 17-23µm² vs 门槛 5.71,原生 4:2
  近似库退出 approx 菜单(exact CT42 结构动作保留)。
- 已知欠账:16 个合格 cell 全部无 dyn 表征(近似常数类当年被表征流程跳过),要做
  功耗打分需补一批 DC 表征(便宜,~16 cell);tmax 无虞(0-2 门)。
- 产物:`outputs/2026-07-11_cell_pareto/substd_{cell_pareto.png,selection.json}`;
  菜单文件 `Appr_Comp/selected_compressors_substd.json`(exact+6/exact+6,可变长加载器
  直接吃);solver 锚点已切原生口径(SlotSpace.exact_area={2.856,2.184,5.712})。

**substd 验证训练(2026-07-11 16:xx 起,`2026-07-11_16_mred_substd_ga_scratch_np4`)**:
用户裁定——**GA v1.1(镜像 2026-07-09_20 配方,无 errgate、无 greedy)**、CT42 关闭、
**从零训练**(无温启动/无 policy 继承),k12/14/16 × C1 预算,ep240/s8/np4/seed42,
config=`_config_errobj_win4_substd.yaml`(仅 approx_lib_path 换 substd 菜单)。
对照=warm180 纯截断(mred-C* 再生版)。两个 1-ep 冒烟(greedy 版+GA 版)均过:
7 个新 cell 的 SOP 模块首次过 DC 无恙,over_budget 0/9。预期管理:substd cell 误差贵
(wae≥0.375/次),k14 冒烟只放得起 ~1 个——本实验回答"真实口径硬保证的 cell 能赚几 µm²"。
旧菜单 greedy 慢版 k14 已杀(与 fast k14 重复);慢版 k12/k16、fast k14 保留作旧菜单臂。
