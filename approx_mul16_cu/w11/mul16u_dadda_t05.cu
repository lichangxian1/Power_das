// ============================================================================
// w11/mul16u_dadda_t05.cu
// 16x16 近似乘法器 -- 纯截断 Dadda 树, 按"激活16bit x 权重11bit"口径标定
//
// 结构与上级目录相同: 丢弃 16x16 部分积矩阵最低 5 列 (列 0..4),
// 其余部分积用标准 Dadda 树精确压缩。
//
// MRED = 1.9e-05  (口径: a ~ U[0,65535], b ~ U[0,2047], 2^24 样本;
...
// 无符号核心: 纯截断 Dadda, 丢弃部分积第 0..4 列
__device__ uint32_t dadda16u_trunc(uint16_t a, uint16_t b)
{
    const uint32_t keep = 0xFFFFFFFFu << 5;
    uint32_t p = 0;
    #pragma unroll
    for (int i = 0; i < 16; ++i)
        if ((a >> i) & 1u)
